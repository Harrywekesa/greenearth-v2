...
if($initiative_id > 0) {
    ...
    if(!$initiative) {
        header("Location: ?page=admin/initiatives/list");
        exit;
    }
} else {
    header("Location: ?page=admin/initiatives/list");
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connection->prepare("UPDATE initiatives SET title = ?, slug = ?, description = ?, objectives = ?, start_date = ?, end_date = ?, location = ?, latitude = ?, longitude = ?, partner_id = ?, target_trees = ?, planted_trees = ?, status = ?, image = ? WHERE id = ?");
    $stmt->bind_param("ssssssssdiiddsi", $title, $slug, $description, $objectives, $start_date, $end_date, $location, $latitude, $longitude, $partner_id, $target_trees, $planted_trees, $status, $image, $initiative_id);
        ...
        <div class="flex flex-col h-full pt-16">
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <nav class="mt-5 flex-1 px-2 space-y-1">
                    ...
                    <a href="?page=admin/initiatives/list" class="bg-green-100 text-green-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
        ...
        <div class="px-4 md:px-10 py-8">
            <div class="max-w-3xl mx-auto">
                <div class="flex justify-between items-center">
                    ...
                    <a href="?page=admin/initiatives/list" class="text-sm font-medium text-green-600 hover:text-green-500">
                        ← Back to initiatives
                    </a>
                </div>
