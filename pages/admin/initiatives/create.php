...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connection->prepare("INSERT INTO initiatives (title, slug, description, objectives, start_date, end_date, location, latitude, longitude, partner_id, target_trees, status, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssdiiss", $title, $slug, $description, $objectives, $start_date, $end_date, $location, $latitude, $longitude, $partner_id, $target_trees, $status, $image);
    
    if($stmt->execute()) {
        $message = "Initiative created successfully!";
        header("Location: ?page=admin/initiatives/list&message=" . urlencode($message));
        exit;
    } else {
