...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connection->prepare("INSERT INTO partners (name, email, phone, address, website, logo, description, sponsorship_level, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssi", $name, $email, $phone, $address, $website, $logo, $description, $sponsorship_level, $is_active);
    
    if($stmt->execute()) {
        $message = "Partner created successfully!";
        header("Location: ?page=admin/partners/list&message=" . urlencode($message));
        exit;
    } else {
        $error = "Error creating partner: " . $stmt->error;
    }
}
?>
...
<head>
    ...
    <title>Add Partner - GreenEarth Admin</title>
        ...
        <div class="flex flex-col h-full pt-16">
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <nav class="mt-5 flex-1 px-2 space-y-1">
                    ...
                    <a href="?page=admin/partners/list" class="bg-green-100 text-green-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
        ...
        <div class="px-4 md:px-10 py-8">
            <div class="max-w-3xl mx-auto">
                <div class="flex justify-between items-center">
                    <h1 class="text-2xl font-bold text-gray-900">Add Partner</h1>
                    <a href="?page=admin/partners/list" class="text-sm font-medium text-green-600 hover:text-green-500">
                        ← Back to partners
                    </a>
                </div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <textarea id="address" name="address" rows="2" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"></textarea>
                            </div>
                            
                            <div class="sm:col-span-3">
                                <label for="website" class="block text-sm font-medium text-gray-700">Website</label>
                                <input type="url" name="website" id="website" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
