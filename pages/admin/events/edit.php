...
if($event_id > 0) {
    ...
    if(!$event) {
        header("Location: ?page=admin/events/list");
        exit;
    }
} else {
    header("Location: ?page=admin/events/list");
...
// Get initiatives for dropdown
$stmt = $connection->prepare("SELECT id, title FROM initiatives WHERE status IN ('upcoming', 'ongoing') ORDER BY title");
$stmt->execute();
$result = $stmt->get_result();
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connection->prepare("UPDATE events SET title = ?, slug = ?, description = ?, event_date = ?, location = ?, latitude = ?, longitude = ?, initiative_id = ?, max_volunteers = ?, status = ?, image = ? WHERE id = ?");
    $stmt->bind_param("ssssssdiissi", $title, $slug, $description, $event_date, $location, $latitude, $longitude, $initiative_id, $max_volunteers, $status, $image, $event_id);
...
<head>
    ...
    <title>Edit Event - GreenEarth Admin</title>
        ...
        <div class="flex flex-col h-full pt-16">
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <nav class="mt-5 flex-1 px-2 space-y-1">
                    ...
                    <a href="?page=admin/events/list" class="bg-green-100 text-green-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
        ...
        <div class="px-4 md:px-10 py-8">
            <div class="max-w-3xl mx-auto">
                <div class="flex justify-between items-center">
                    ...
                    <a href="?page=admin/events/list" class="text-sm font-medium text-green-600 hover:text-green-500">
                        ← Back to events
                    </a>
                </div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            <div class="sm:col-span-6">
                                ...
                                <input type="text" name="title" id="title" value="<?php echo htmlspecialchars($event['title']); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-6">
                                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                                <textarea id="description" name="description" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"><?php echo htmlspecialchars($event['description']); ?></textarea>
                            </div>
                            
                            <div class="sm:col-span-3">
                                <label for="event_date" class="block text-sm font-medium text-gray-700">Event Date & Time</label>
                                <input type="datetime-local" name="event_date" id="event_date" value="<?php echo date('Y-m-d\TH:i', strtotime($event['event_date'])); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <select id="initiative_id" name="initiative_id" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                                    ...
                                    <option value="<?php echo $initiative['id']; ?>" <?php echo $initiative['id'] == $event['initiative_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($initiative['title']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="sm:col-span-4">
                                ...
                                <input type="text" name="location" id="location" value="<?php echo htmlspecialchars($event['location']); ?>" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-1">
                                <label for="latitude" class="block text-sm font-medium text-gray-700">Latitude</label>
                                <input type="text" name="latitude" id="latitude" value="<?php echo $event['latitude']; ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-1">
                                <label for="longitude" class="block text-sm font-medium text-gray-700">Longitude</label>
                                <input type="text" name="longitude" id="longitude" value="<?php echo $event['longitude']; ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-3">
                                <label for="max_volunteers" class="block text-sm font-medium text-gray-700">Maximum Volunteers</label>
                                <input type="number" name="max_volunteers" id="max_volunteers" min="1" value="<?php echo $event['max_volunteers']; ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <?php if($event['image']): ?>
                                <div class="mt-2">
                                    <img src="../../../uploads/events/<?php echo $event['image']; ?>" alt="Current image" class="h-20 w-20 object-cover rounded-md">
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>
