...
if($user_id > 0) {
    ...
    if(!$user) {
        header("Location: ?page=admin/users/list");
        exit;
    }
} else {
    header("Location: ?page=admin/users/list");
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($result->num_rows > 0) {
        ...
        if(!$error) {
            if($update_password) {
                $stmt = $connection->prepare("UPDATE users SET name = ?, email = ?, phone = ?, password = ?, county = ?, subcounty = ?, role = ?, is_active = ? WHERE id = ?");
                $stmt->bind_param("ssssssssi", $name, $email, $phone, $hashed_password, $county, $subcounty, $role, $is_active, $user_id);
            } else {
                $stmt = $connection->prepare("UPDATE users SET name = ?, email = ?, phone = ?, county = ?, subcounty = ?, role = ?, is_active = ? WHERE id = ?");
                $stmt->bind_param("ssssssii", $name, $email, $phone, $county, $subcounty, $role, $is_active, $user_id);
            }
...
<head>
    ...
    <title>Edit User - GreenEarth Admin</title>
        ...
        <div class="flex flex-col h-full pt-16">
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <nav class="mt-5 flex-1 px-2 space-y-1">
                    ...
                    <a href="?page=admin/users/list" class="bg-green-100 text-green-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                    ...
                    <form method="POST">
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <input type="text" name="county" id="county" value="<?php echo htmlspecialchars($user['county']); ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-3">
                                <label for="subcounty" class="block text-sm font-medium text-gray-700">Subcounty</label>
                                <input type="text" name="subcounty" id="subcounty" value="<?php echo htmlspecialchars($user['subcounty']); ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <input type="password" name="password" id="password" minlength="8" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-6">
                                <label for="confirm_password" class="block text-sm font-medium text-gray-700">Confirm New Password</label>
                                <input type="password" name="confirm_password" id="confirm_password" minlength="8" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="sm:col-span-6">
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="is_active" name="is_active" type="checkbox" <?php echo $user['is_active'] ? 'checked' : ''; ?> class="focus:ring-green-500 h-4 w-4 text-green-600 border-gray-300 rounded">
                                    </div>
