...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // SOLUTION: Bypass datetime issues by using NULL for published_at
    $published_at = null;
    
    // If post is published, set to current timestamp
    if($is_published) {
        $published_at = date('Y-m-d H:i:s');
    }
    
    // Check if published_at column exists
    $stmt = $connection->prepare("SHOW COLUMNS FROM blog_posts LIKE 'published_at'");
    $stmt->execute();
    $result = $stmt->get_result();
    
    if($result->num_rows > 0) {
    // Column exists, but we won't pass published_at — let MySQL handle it
    $stmt = $connection->prepare("INSERT INTO blog_posts (title, slug, excerpt, content, featured_image, author_id, meta_description, meta_keywords, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if($stmt->execute()) {
        ...
        header("Location: ?page=admin/blog/list&message=" . urlencode($message));
        exit;
    } else {
...
<head>
    ...
    <title>Create Blog Post - GreenEarth Admin</title>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea id="excerpt" name="excerpt" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"></textarea>
                            </div>
                            
                            <div>
                                <label for="content" class="block text-sm font-medium text-gray-700">Content</label>
                                <textarea id="content" name="content" rows="15" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"></textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="meta_description" name="meta_description" rows="2" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"></textarea>
                            </div>
                            
                            <div>
                                <label for="meta_keywords" class="block text-sm font-medium text-gray-700">Meta Keywords</label>
                                <input type="text" name="meta_keywords" id="meta_keywords" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                            </div>
                            
                            <div class="flex items-center">
                                <input id="is_published" name="is_published" type="checkbox" class="focus:ring-green-500 h-4 w-4 text-green-600 border-gray-300 rounded">
