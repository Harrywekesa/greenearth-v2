...
<?php
// Don't start session again if already started
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}
// No need to require config since it's already included in index.php
// Just check if we have access to database connection
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check credentials - using global connection from index.php
    global $connection;
    
    if ($connection) {
        $stmt = $connection->prepare("SELECT id, name, email, role FROM users WHERE email = ? AND role IN ('admin', 'partner') AND is_active = 1");
        if ($stmt) {
            ...
            if($user = $result->fetch_assoc()) {
                // In a real implementation, verify password with password_verify()
    ...
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            ...
            <div class="text-center text-sm text-gray-500">
                <p>Demo credentials: admin@greenearth.org / admin123</p>
            </div>
        </div>
    </div>
</body>
